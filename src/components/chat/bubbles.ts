...
export default class ChatBubbles {
  ...
  private async renderMessage({
    ...
    if(needToSetHTML) {
      ...
      const langFormat = this.chat.appSettings.languageFormat;
      if(langFormat?.enabled && context.messageMessage && !bigEmojis) {
        const targetLangFull = our ? langFormat.clientLanguage : langFormat.myLanguage;
        const targetLang = targetLangFull?.split('-')[0];
        if(targetLang) {
          const sourceText = context.messageMessage;
          const sourceEntities = totalEntities || [];
          const translatePromise = (this.managers.appTranslationsManager.translateText({
            text: {_: 'textWithEntities', text: sourceText, entities: sourceEntities},
            lang: targetLang
          }) as Promise<TextWithEntities>);

          translatePromise.then((translated) => {
            if(!middleware()) return;
            if(!translated || !translated.text || translated.text === sourceText) return;

            const translatedSpan = document.createElement('span');
            translatedSpan.classList.add('bubble-translated-text');
            const translatedRich = wrapRichText(translated.text, getRichTextOptions(translated.entities));
            setInnerHTML(translatedSpan, translatedRich);

            const divider = document.createElement('hr');
            divider.classList.add('bubble-translation-divider');

            if(our) {
              messageDiv.prepend(translatedSpan, divider);
            } else {
