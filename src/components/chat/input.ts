...
export default class ChatInput {
  ...
  public async sendMessage(force = false) {
    ...
    if(langFormat?.enabled && trimmedValue && !editMsgId) {
      const targetLang = langFormat.clientLanguage?.split('-')[0];
      if(targetLang) {
        try {
          const translated = await this.managers.appTranslationsManager.translateText({
            text: {_: 'textWithEntities', text: value, entities: entities || []},
            lang: targetLang
          });
